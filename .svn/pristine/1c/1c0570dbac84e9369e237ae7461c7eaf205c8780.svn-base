...
import com.wanding.face.bean.AuthRefreshOrderStateResData;
...
//                    speak("扫码成功！");
        ...
        speak("预授权"+totalFee+"元，请出示付款码或点击刷脸支付！");
...
//        return super.dispatchKeyEvent(event);
    ...
    private void queryOrderStatus(){
        if(authResultResponse != null){
            ...
            //参数实体
            AuthRefreshOrderStateReqData reqData = QueryParamsReqUtil.refreshOrderStateReq(userBean,orderId);
            //发起查询请求
    ...
    private void refreshOrderStateStr(String jsonStr){
        try{
            Gson gjson  =  GsonUtils.getGson();
            AuthRefreshOrderStateResData resData = gjson.fromJson(jsonStr, AuthRefreshOrderStateResData.class);
            //return_code	响应码：“01”成功 ，02”失败，请求成功不代表业务处理成功
